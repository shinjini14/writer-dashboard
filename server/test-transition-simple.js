...
// Simple test for June 5th to June 6th transition without PostgreSQL
async function testTransitionSimple() {
  console.log('🧪 Testing June 5th → June 6th Transition (Simple)');
  console.log('==================================================');

  try {
    const writerId = 110;
    const writerName = 'Shannen Santiago'; // Hardcode to avoid PostgreSQL
    ...
    if (june6thIncrease < 0) {
      ...
      console.log(`   This suggests data inconsistency between sources`);
      ...
      console.log(`   This suggests the transition calculation is still wrong`);
    ...
    // Step 5: Get some context from recent InfluxDB daily increases
    console.log('\n📊 STEP 5: Compare with recent InfluxDB daily increases');
    ...
    try {
      ...
      console.log(`📊 Recent InfluxDB daily increases:`);
      influxResults.slice(-3).forEach(day => {
      ...
      if (june5thInfluxIncrease) {
        ...
        if (ratio > 10) {
          console.log(`   ⚠️ WARNING: June 6th is ${ratio.toFixed(1)}x higher than June 5th`);
        } else if (ratio < 0.1) {
          console.log(`   ⚠️ WARNING: June 6th is ${(1/ratio).toFixed(1)}x lower than June 5th`);
        } else {
          console.log(`   ✅ GOOD: Reasonable ratio between consecutive days`);
        }
      }
    } catch (influxError) {
      console.log(`⚠️ Could not get InfluxDB context:`, influxError.message);
    }
    ...
    if (june6thIncrease > 0 && june6thIncrease < 1000000) {
      console.log(`🎯 RESULT: Transition calculation appears CORRECT`);
    } else {
      console.log(`❌ RESULT: Transition calculation needs further investigation`);
    }
